...
if __name__ == '__main__':
    ...
    print("Label dictionary items:")
    print(label_dict.items())
    num_class = len(label_dict)

    dataset = BertDataset(device=device, pad_idx=tokenizer.pad_token_id, data_path=data_path)
    model = ContrastModel.from_pretrained('bert-base-multilingual-uncased', num_labels=num_class,
    ...
    with torch.no_grad():
        for data, label, idx in pbar:
            ...
            for l in output['logits']:
                ...
                # print(f'Predicted: {torch.sigmoid(l).tolist()}')
    ...
    print('macro_f1', macro_f1, 'micro_f1', micro_f1, 'precision', precision, 'recall', recall)
